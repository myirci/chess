	...
	private:
		...
		void GenerateKingMoves(const BoardType& board, Square king_pos, MoveList& moves)
		{
			...
			for (Direction dir : BoardType::AllDirections)
			{
					...
					!_attackDetector.IsUnderAttack<ctraits::Opposite>(board, next))
				{
					...
					else if (color::get_color(brd[next]) != SideToMove)
			...
			if (board.QueryCastling(ctraits::KingSideCastling) &&
				brd[bctraits::KingSideCastleCheckSquares[0]] == Empty &&
				brd[bctraits::KingSideCastleCheckSquares[1]] == Empty &&
			...
			if (board.QueryCastling(ctraits::QueenSideCastling) &&
				brd[bctraits::QueenSideCastleCheckSquares[0]] == Empty &&
				brd[bctraits::QueenSideCastleCheckSquares[1]] == Empty &&
				brd[bctraits::QueenSideCastleCheckSquares[2]] == Empty &&
		...
		void GenerateCheckerCapturingMoves(const BoardType& board, MoveList& moves) 
		{
			...
			for (Direction dir : BoardType::StraightDirections)
			{
				...
				for (Square next{ check.checker + dir }; IsInside<BoardType>(next, next - dir); next += dir)
				{
					...
					if ((brd[next] == ctraits::Rook || brd[next] == ctraits::Queen) && !_attackDetector.IsPinned(next))
			...
			for (Direction dir : BoardType::DiagonalDirections)
			{
				...
				for (Square next{ check.checker + dir }; IsInside<BoardType>(next, next - dir); next += dir)
				{
					...
					if ((brd[next] == ctraits::Bishop || brd[next] == ctraits::Queen) && !_attackDetector.IsPinned(next))
						moves.emplace_back(next, check.checker, MoveType::Capture);
			...
			{
				if (Square next{ check.checker + dir }; IsInside<BoardType>(next, check.checker) && brd[next] == ctraits::Knight && !_attackDetector.IsPinned(next))
					moves.emplace_back(next, check.checker, MoveType::Capture);
			}
			...
			if (check.checker >= bctraits::ValidPawnMoveSquares[0] && check.checker <= bctraits::ValidPawnMoveSquares[1])
			{
				...
				{
					if (Square next{ check.checker + dir }; IsInside<BoardType>(next, check.checker) && brd[next] == ctraits::Pawn && !_attackDetector.IsPinned(next))
					{
				...
				if (enpassant_sq != Empty && brd[check.checker] == octraits::Pawn)
				{
					Square next{ check.checker - 1 };
					for (int i = 0; i < 2; i++) 
					{
						if (IsInside<BoardType>(enpassant_sq, next) && brd[next] == ctraits::Pawn && !_attackDetector.IsPinned(next))
		...
		void GenerateCheckBlockingMoves(const BoardType& board, MoveList& moves) const
		{
			...
			for (Distance i{ 1 }; i < check.checker_dist; i++, sq += check_dir)
			{
				...
				for (Direction dir : BoardType::KnightJumps) 
				{
					if (Square next{ sq + dir }; IsInside<BoardType>(next, sq) && brd[next] == ctraits::Knight && !_attackDetector.IsPinned(next))
						moves.emplace_back(next, sq, MoveType::Quite);
				}
				...
				if (sq >= bctraits::ValidPawnMoveSquares[0] && sq <= bctraits::ValidPawnMoveSquares[1])
				{
					...
					if (brd[next] == ctraits::Pawn && !_attackDetector.IsPinned(next))
					{
					...
					else if (brd[next] == Empty)
					{
						next += bctraits::PawnReverseMoveDirection;
						if (brd[next] == ctraits::Pawn && !_attackDetector.IsPinned(next) && BoardType::GetRank(next) == bctraits::PawnDoublePushRank)
		...
		void GenerateKnightMoves(const BoardType& board, MoveList& moves) const
		{
			...
			for (const auto& brd = board.GetBoard(); first != last; first++)
			{
		...
		inline void GenerateStraightMoves(const BoardType& board, Square pos, Direction dir, MoveList& moves) const 
		{
			...
			for (Square next{ pos + dir }; IsInside<BoardType>(next, next - dir); next += dir)
			{
				...
				{
					if (color::get_color(brd[next]) != SideToMove)
		...
		void GeneratePawnMoves(const BoardType& board, MoveList& moves) const
		{
			...
			for (; first != last; first++)
			{
					...
					(pin_dir == Empty || pin_dir == bctraits::PawnMoveDirection || pin_dir == bctraits::PawnReverseMoveDirection))
				{
					...
					if (Square pos{ next + bctraits::PawnMoveDirection }; 
						BoardType::GetRank(first->second) == bctraits::PawnDoublePushRank && brd[pos] == Empty)
				...
				for (int i{ 0 }; i < 2; i++)
				{
					if (Square next{ first->second + bctraits::PawnAttackDirections[i] }; 
						...
						brd[next] != Empty && 
						color::get_color(brd[next]) != SideToMove && 
			...
			for (int i{ 0 }; i < 2; i++)
			{
				...
				if (!IsInside<BoardType>(en_passant, pos) || brd[pos] != ctraits::Pawn)
					continue;
				...
				else
				{
					...
					{
						if (next == pos || next == ppos || brd[next] == Empty)
							continue;

						if (brd[next] == octraits::Rook || brd[next] == octraits::Queen)
							make_move = false;
						break;
					}
